...
export function Dashboard({ onLogout }: { onLogout: () => void }) {
	...
	const { uploadQueue, setUploadQueue, handleManualUpload, isDragging } =
		useFileUpload(activeFolderId, store);
	const { downloadQueue, clearFinished: clearDownloads } =
	...
	return (
		...
		>
			...
			<UploadQueue
				...
				onClearFinished={() =>
					setUploadQueue((q) =>
						q.filter((i) => i.status !== "success" && i.status !== "error"),
					)
				}
			/>
			<DownloadQueue items={downloadQueue} onClearFinished={clearDownloads} />
		</div>
	);
}
