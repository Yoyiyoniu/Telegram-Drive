...
export function MediaPlayer({
	...
	const folderIdParam =
		activeFolderId !== null ? activeFolderId.toString() : "home";
	const streamUrl = streamToken
		? `http://localhost:14200/stream/${folderIdParam}/${file.id}?token=${streamToken}`
		: null;

	const isVideo = ["mp4", "webm", "ogg", "mov", "mkv", "avi"].some((ext) =>
		file.name.toLowerCase().endsWith(ext),
	);
	const isAudio = ["mp3", "wav", "aac", "flac", "m4a", "opus"].some((ext) =>
		file.name.toLowerCase().endsWith(ext),
	);
				...
				<div className="w-full aspect-video bg-black rounded-xl overflow-hidden shadow-2xl ring-1 ring-white/10 flex items-center justify-center">
					{!streamUrl ? (
						...
						<div className="w-full h-full flex flex-col items-center justify-center bg-gradient-to-br from-telegram-primary/20 to-black">
							<div className="w-32 h-32 rounded-full glass flex items-center justify-center mb-8 shadow-xl animate-pulse-slow border border-telegram-primary/30">
								<svg
				...
				<div className="mt-4 text-center">
					...
					<p className="text-sm text-white/50">
						...
						{typeof currentIndex === "number" &&
							...
							totalItems > 0 && (
								<span className="ml-2">
									• {currentIndex + 1}/{totalItems} • Arrow/J-L
								</span>
							)}
					</p>
				</div>
